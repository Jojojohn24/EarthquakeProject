...
def start_pipeline():
    with beam.Pipeline(options=options) as p:
        ...
        _ = (
            ...
            FetchAndUpload(config.hist_url, config.bucket_name, config.DATAFLOW_LANDING_LOCATION))
        )
    ...
    with beam.Pipeline(options=options) as p:
        trigger_and_data = (
            ...
            | 'Read JSON from GCS' >> beam.io.ReadFromText(
                f"gs://{config.bucket_name}/{config.DATAFLOW_LANDING_LOCATION}")
        ...
        silver_data | 'Write Transformed Data to Parquet (Silver Layer)' >> WriteToParquet(
            file_path_prefix=f'gs://{config.bucket_name}/{config.DATAFLOW_SILVER_LAYER_PATH}',
...
def pipeline_for_bigquery():
    with beam.Pipeline(options=options) as p:
        ...
        read_parquet = (
            ...
            f'gs://{config.bucket_name}/{config.DATAFLOW_SILVER_LAYER_PATH}-00000-of-00001.parquet')
